...
<main class="col bg-faded py-3 flex-grow-1">
	...
	<div class="card">
        <div class="card-header">Cập nhật sự kiện</div>
        @if ($errors->any())
        ...
        <div class="card-body">
            ...
            @endif
            <form method="POST" action="{{route('sukien.update',[$event->id])}}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Tên sự kiện</label>
                    <input type="text" class="form-control" value="{{$event->tensukien}}"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{$event->slug_sukien}}" name="slug_sukien"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{$event->diadiem}}" name="diadiem" aria-describedby="emailHelp"
                        placeholder="Địa điểm...">
                </div>

                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">chi tiết sự kiện</label>
                    <textarea class="form-control" id="chitietsukien" name="chitietsukien" style="resize: none" rows="5">{{$event->chitietsukien}}</textarea>
                </div>

                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Hình ảnh sự kiện</label>
                    <input type="file" class="form-control-file" name="image">
                    <img src="{{asset('public/uploads/sukien/'.$event->image)}}" height="150" width="150">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{$event->giave}}" name="giave" aria-describedby="emailHelp" placeholder="Giá vé...">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="tinhtrang" class="form-select" aria-label="Default select example">
                        @if ($event->tinhtrang==0)
                            <option selected value="0">Đang diễn ra</option>
                            <option value="1">Sự kiện đã kết thúc</option>
                        @else
                            <option value="0">Đang diễn ra</option>
                            <option selected value="1">Sự kiện đã kết thúc</option>
                        @endif
                    </select>
                </div>
